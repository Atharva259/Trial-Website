...
export default function PosPage() {
  ...
  // Drawer input states
  const [openingBalanceInput, setOpeningBalanceInput] = useState('2500'); // Rs 2500 opening float
  ...
  const getPosCartTotal = () => {
    ...
    return Math.round(sub * 1.05 * 100) / 100; // 5% GST
  };
  ...
  const handlePosCheckout = () => {
    ...
    try {
      ...
      // Counter cash transactions are takeaway orders served immediately
      ...
      if (order) {
        ...
        setPosCart({});
        alert(`POS Counter cash order ${order.orderNumber} placed & served successfully!`);
      } else {
        alert('Order failed. Verify ingredient stock.');
      }
    } catch (err: any) {
      alert(err.message || 'Checkout failed due to ingredient bounds.');
    }
  };
      ...
      <div className="smartphone-frame">
        ...
        <div className="screen-header">
          <div>
            ...
            <h1 style={{ fontSize: '24px', fontWeight: '800', letterSpacing: '-0.5px', color: 'var(--accent-color)' }}>Counter Sales</h1>
          </div>
        ...
        <div className="screen-content">
          ...
          <div 
            ...
            style={{ 
              ...
              alignItems: 'center',
              borderColor: isDrawerOpen ? 'rgba(22, 163, 74, 0.2)' : 'rgba(220, 38, 38, 0.2)',
              background: isDrawerOpen ? 'rgba(22, 163, 74, 0.02)' : 'rgba(220, 38, 38, 0.02)'
            }}
          >
            <div>
              <p style={{ fontSize: '11px', fontWeight: '600', color: 'var(--text-secondary)' }}>CASH REGISTER DRAWER</p>
              <h4 style={{ fontSize: '14px', fontWeight: '800', marginTop: '2px' }}>
            ...
            <button
              ...
              style={{
                background: isDrawerOpen ? '#FFFFFF' : 'var(--accent-color)',
                color: isDrawerOpen ? 'var(--text-primary)' : '#FFFFFF',
          ...
          {!isDrawerOpen ? (
            <div style={{ textAlign: 'center', padding: '40px 20px', color: 'var(--text-secondary)' }}>
              <Lock size={48} style={{ opacity: 0.15, marginBottom: '16px' }} />
              <h4 style={{ color: 'var(--text-primary)', fontWeight: '700' }}>POS Drawer Locked</h4>
              <p style={{ fontSize: '12px', marginTop: '6px' }}>Open the cash register drawer float session to manage over-the-counter payments.</p>
            ...
            <div>
              ...
              {Object.keys(posCart).length > 0 && (
                <div className="glass-card" style={{ background: '#FFFFFF', marginBottom: '24px', borderColor: 'var(--accent-color)' }}>
                    ...
                    return (
                      <div key={id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '8px 0', borderBottom: '1px solid var(--border-color)', fontSize: '13px' }}>
                        ...
                        <div style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
                          <button onClick={() => removeFromPosCart(id)} style={{ background: '#F1EFE9', border: 'none', color: 'var(--text-primary)', width: '24px', height: '24px', borderRadius: '4px', cursor: 'pointer' }}>-</button>
                          <button onClick={() => addToPosCart(id)} style={{ background: '#F1EFE9', border: 'none', color: 'var(--text-primary)', width: '24px', height: '24px', borderRadius: '4px', cursor: 'pointer' }}>+</button>
                        </div>
                      </div>
                    );
                  })}
                  <button onClick={handlePosCheckout} className="btn btn-primary" style={{ marginTop: '16px', height: '44px' }}>
                    Collect Cash & Print: ₹{getPosCartTotal().toFixed(2)}
                  </button>
                </div>
              )}
          ...
          <div className="drawer-backdrop" onClick={() => setShowDrawerModal(false)} style={{ zIndex: 2000 }}>
            <div className="drawer-sheet" style={{ zIndex: 2001 }} onClick={(e) => e.stopPropagation()}>
              ...
              <div className="drawer-body">
                {isDrawerOpen ? (
                  <div>
                    <div style={{ background: '#FFFFFF', padding: '14px', borderRadius: '12px', border: '1px solid var(--border-color)', marginBottom: '16px', fontSize: '13px' }}>
                      <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '6px' }}>
                        <span>Opening Float:</span> <span>₹{state.cashSession.openingBalance.toFixed(2)}</span>
                      </div>
                      <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '6px' }}>
              ...
              <div className="drawer-body">
                ...
                <div className="form-group">
                  ...
                  <input 
                    type="text" 
                    className="form-input" 
                    placeholder={adjustmentType === 'cash_in' ? 'e.g. Added change float' : 'e.g. Paid tea leaf vendor'} 
